...
export default function Home() {
  ...
  const handleSubmit = async (event) => {
   ...
   if (selectPost?._id) {
    ...
    const response = await sendRequest(`${BASE_URL}/post/update/${selectPost._id}`, {
      method: 'post',
      payload: formData
    });
  ...
  const handleDelete = async (postId) => {
    try {
      const response = await sendRequest(`${BASE_URL}/post/delete/${postId}`, { method: 'post' });
      if (response.status === 200) {
             ...
             <ul className="list-group list-group-flush">
               <li className="list-group-item">
                 <div className="d-flex justify-content-between">
                   <span className="badge bg-primary text-capitalize"> {post.userID?.name}</span>
                 ...
                 {post.userID?.role == "admin"  &&
                  <>
                <span className="badge bg-warning"  type="button" onClick={() => handleEdit(post._id)}>Edit Post</span>
                <span className="badge bg-danger"  type="button" onClick={() => handleDelete(post._id)}>Delete Post</span>
                  </>
                  }
